...
f = st.file_uploader("Importer GPX", type="gpx")
...
if f:
    ...
    if r:
        ...
        st.header("🏁 Indicateurs de l'effort")
        c1, c2, c3 = st.columns(3)
        c1.metric("Temps Total", f"{h}h {m}min")
        c2.metric("P. Moy. Totale", f"{tw/tt:.0f} W" if tt>0 else "0 W")
        c3.metric("Distance Totale", f"{d_t/1000:.1f} km")
        ...
        c2.metric("Énergie Totale", f"{tw/1000:.0f} kJ")
        c3.metric("Altitude Max", f"{a_m:.0f} m")
        ...
        fig_p.update_layout(xaxis_title="Distance (km)", yaxis_title="Altitude (m)", height=300)
        st.plotly_chart(fig_p, use_container_width=True)

        ca, cb = st.columns(2)
        with ca:
            st.subheader("📊 Répartition du Terrain")
            df_t = pd.DataFrame([{"Type":k, "Km":v["d"]/1000, "%":(v["t"]/tt)*100 if tt>0 else 0} for k,v in st_t.items()])
            st.table(df_t.style.format({"Km":"{:.1f}", "%":"{:.1f}%"}))
            st.plotly_chart(px.pie(df_t, names="Type", values="Km", hole=0.4, height=350), use_container_width=True)
        with cb:
            st.subheader("🌬️ Répartition des Résistances")
            tr_v = sum(res.values())
            df_r = pd.DataFrame([{"Source":k, "kJ":v/1000, "%":(v/tr_v)*100 if tr_v>0 else 0} for k,v in res.items()])
            st.table(df_r.style.format({"kJ":"{:.0f}", "%":"{:.1f}%"}))
            st.plotly_chart(px.pie(df_r, names="Source", values="kJ", hole=0.4, height=350), use_container_width=True)