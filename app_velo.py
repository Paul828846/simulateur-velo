...
# --- CSS PERSONNALISÉ POUR SMARTPHONE (CORRIGÉ) ---
st.markdown("""
    <style>
    ...
    /* Réduit l'espace entre les indicateurs sur mobile */
...
def analyze_gpx(file, vf_k, vc_k, vd_k, wr, wb, v_wind_k):
    ...
    for tr in g.tracks:
        for sg in tr.segments:
            for p in sg.points: 
                all_p.append([p.latitude, p.longitude, p.elevation or 0])
...
with st.expander("📖 Guide d'utilisation et aide"):
    st.info("**📱 Note mobile :** Cliquez sur la flèche en haut à gauche pour régler poids, vent et vitesses.")
    st.write("""
    Cette application simule l'effort nécessaire pour votre parcours :
    * **Poids :** Impacte l'effort en montée.
    * **Vent :** [+] Face / [-] Dos. Influence l'aérodynamisme.
    * **Vitesses :** Définissez vos allures pour estimer la puissance requise.
    """)
...
if f:
    ...
    if r:
        ...
        fig_p.update_layout(xaxis_title="Distance (km)", yaxis_title="Altitude (m)", height=300, margin=dict(l=0, r=0, t=20, b=0))
